...
import { getNoticeData, getNoticeDetails, removeNotice } from '@/api/noticeManage'
...
export default {
  ...
  data() {
    return {
      ...
      tableRow: [
        { key: 'supplierName', label: '编辑' },
        { key: 'supplierUsername', label: '标题' },
        { key: 'supplierPhone', label: '创建时间' },
        {
          key: 'operate',
          label: '操作',
          btn: [
  ...
  methods: {
    ...
    async getData() {
      ...
      const res = await getNoticeData(this.queryParam, paging)
