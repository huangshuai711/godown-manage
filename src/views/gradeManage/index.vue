...
import { getGradeData, importStrtegy } from '@/api/gradeManage'
export default {
  name: 'commodity-info',
  ...
  data() {
    return {
      ...
      tableRow: [
        { key: 'smokeName', label: '文件名' },
        { key: 'smokeSource', label: '排序' },
        { key: 'createTime', label: '创建时间' },
        {
          key: 'operate',
          label: '操作',
          btn: [{ key: 'details', name: '查看文件' }]
        }
      ],
      tableData: [],
      total: 0,
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        const res = await getGradeData(this.queryParam, paging)
