...
export default {
  ...
  data() {
    return {
      ...
      tableRow: [
        { key: 'documentNum', label: '销售单据号' },
        { key: 'createUserName', label: '销售人姓名' },
        { key: 'totalCount', label: '销售总数量' },
  ...
  methods: {
    ...
    async export() {
      try {
        const res = await exportPurchases({ ...this.queryParam, documentType: 3 })
        downloadFile(res, '销售单据信息')
      } catch (error) {}
    },
    add() {
    ...
    async getData() {
      try {
        ...
        const res = await getPurchasesList({ ...this.queryParam, documentType: 3 }, paging)
