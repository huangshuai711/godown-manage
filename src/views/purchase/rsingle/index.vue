...
import { getPurchasesList, exportCommodity } from '@/api/purchase'
...
export default {
  ...
  methods: {
    ...
    async export() {
      try {
        const res = await exportPurchases({ ...this.queryParam, documentType: 2 })
    ...
    async getData() {
      try {
        ...
        const res = await getPurchasesList({ ...this.queryParam, documentType: 2 }, paging)
