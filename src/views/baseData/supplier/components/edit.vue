...
import { addSupplier, editSupplier } from '@/api/baseData'
export default {
  ...
  data() {
    return {
      ...
      formArr: [
        { type: 'input', label: '供应商名称', prop: 'supplierName', req: true },
        { type: 'input', label: '负责人名称', prop: 'supplierUsername', req: true },
        { type: 'input', label: '负责人电话', prop: 'supplierPhone', req: true },
        { type: 'address', label: '供应商地址', prop: 'supplierAddress', req: true }
      ],
      data: null
    }
  },
  ...
  methods: {
    ...
    async confirm() {
      try {
        const inter = this.data ? editSupplier : addSupplier
