...
import { addClient, editClient } from '@/api/baseData'
export default {
  ...
  data() {
    return {
      ...
      formArr: [
        { type: 'input', label: '客户名称', prop: 'clientName', req: true },
        { type: 'input', label: '负责人名称', prop: 'clientUsername', req: true },
        { type: 'input', label: '负责人电话', prop: 'clientPhone', req: true },
        { type: 'address', label: '客户地址', prop: 'clientAddress', req: true }
      ],
      data: null
    }
  },
  ...
  methods: {
    ...
    async confirm() {
      try {
        const inter = this.data ? editClient : addClient
