  ...
  data() {
    return {
      ...
      tableRow: [
        ...
        { key: 'productTypeStr', label: '商品类别' },
        { key: 'stripCodeValue', label: '条形码值' },
        { key: 'stripCodePath', label: '条形码图片', type: 'imger' },
  ...
  methods: {
    ...
    remove() {
      this.$confirm(`确认删除？`, '提示', {
        ...
        .then(() => {
          removeCommodity(row.id).then(res => {
            if (res.code == 200) {
    ...
    async goDetails(row) {
      this.$refs.detail.data = await getCommodityDetails(row.id).then(res => res.data)
    ...
    async getData() {
      try {
        ...
        const res = await getCommodityData(this.queryParam, paging)
